...
def evaluate_single_model_class(model : BaseEstimator, xtrain : pd.DataFrame, ytrain : pd.Series, xtest : pd.DataFrame, ytest : pd.Series) -> dict:
    # Cross validate
    scores = cross_val_score(model, xtrain, ytrain, cv=5, scoring="f1_macro")
    cv_mean = scores.mean().round(4)
    cv_std = scores.std().round(4)
    #fit the model
    model.fit(xtrain, ytrain) # type: ignore
    # Predict results for train and test
    ypred_train = model.predict(xtrain) # type: ignore
    ypred_test = model.predict(xtest) # type: ignore
...
def algo_evaluation(
    ...
    results = []
    # Apply for loop on all the models
    for model in models:
    ...
    return best_model, sort_df # type: ignore
